...
abstract class Filler_for_entity extends \Pokeliga\Data\Filler
{
...
class Filler_for_entity_generic extends Filler_for_entity
{
	...
	public function run_step()
	{
		...
		elseif ($this->step===static::STEP_DETERMINE_FILLER) // этот шаг, наконец, пытается заполнить содержиое сущности. WIP! выбор генератора или генераторов может быть более сложный: попытка прочесть, попытка вычислить, попытка заполнить по умолчанию...
		{
			...
			(
				...
				($this->original_state===Value_of_entity::STATE_IRRELEVANT) ||	// или если значение устарело...
				(empty($keeper=$this->value->get_keeper_task()))		// или если значение не является хранимым.
			)
			{
				$result=$this->determine_generator($this->value);
			...
			// если значение хранится, а не генерируется...
			{
				...
				$this->source_type=Value_of_entity::BY_KEEPER;
				$this->filler=$keeper;
				$report=$keeper->load();
				return $report;
			}
	...
	{
		if ($this->value->mode===\Pokeliga\Data\Value::MODE_CONST)
		{
			$this->source_type=\Pokeliga\Data\Value::NEUTRAL_CHANGE;
			return $this->sign_report(new \Report_resolution($this->value_model_now('const')));
		}
		
		$filler=$this->value->get_generator_task();
		if (!empty($filler))
		{
			$filler->source_type=\Pokeliga\Data\Value::NEUTRAL_CHANGE;
...
// для значения из класса \Pokeliga\Data\Value_entity
...
// для значения из класса \Pokeliga\Data\Value_reference
