...
class PHPWebSocket
{
	...
	public $chat_server; // added by EvilCat 24 Apr 2016
	...
	*/
	...
	// added by EvilCat 24 Apr 2016
	function registerChatServer(ChatServer $chat)
	{
		$this->chat_server=$chat;
	}
	
	// added by EvilCat 24 Apr 2016
	function makeExternalCall(Callable $call, ...$args)
	{
		try
		{
			$call(...$args);
		...
		{
			if (!empty($this->chat_server)) $this->char_server->handle_exception($e);
			else throw $e;
		}
	}
	...
	function wsStartServer($host, $port) {
		...
		while (isset($this->wsRead[0])) {
			...
			{
				$time=time();
				foreach ($this->timeouts as $id=>$timeout_data)
				{
					if ($timeout_data[1]>$time) continue;
					
					$call=$timeout_data[0];
					$this->clearTimeout($id);
					$this->makeExternalCall($call);
				}
			}
		}
	...
	function wsRemoveClient($clientID) {
		...
		if ( array_key_exists('close', $this->wsOnEvents) )
			foreach ( $this->wsOnEvents['close'] as $func )
				$this->makeExternalCall($func, $clientID, $closeStatus);
			...
			if ($result) {
				...
				if ( array_key_exists('open', $this->wsOnEvents) )
					foreach ( $this->wsOnEvents['open'] as $func )
						$this->makeExternalCall($func, $clientID);
			}
		}
		else {
		...
		elseif ($opcode == self::WS_OPCODE_TEXT || $opcode == self::WS_OPCODE_BINARY) {
			if ( array_key_exists('message', $this->wsOnEvents) )
				foreach ( $this->wsOnEvents['message'] as $func )
					$this->makeExternalCall($func, $clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY);
		}
		else {
			// unknown opcode
			return false;
	...
	// visibility changed by EvilCat 24 Apr 2016
	...
	// added by EvilCat
	public $timeouts=[], $next_timeout_id=0;
	function setTimeout(Callable $call, $timeout, $id=null)
	{
		if ($id!==null and array_key_exists($id, $this->timeouts)) throw new Exception();
		elseif ($id===null) while ($id===null or array_key_exists($id, $this->timeouts)) $id=$this->next_timeout_id++;
		
		$time=microtime(true)+$timeout;
		$this->timeouts[$id]=[$call, $time];
		uasort($this->timeouts, function($data1, $data2) { if ($data1[1]===$data2[1]) return 0; if ($data1[1]<$data2[1]) return -1; else return 1; });
		return $id;
	}
