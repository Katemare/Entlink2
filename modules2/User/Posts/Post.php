...
class Post extends EntityType
{
	...
	static
		...
		[
			...
			Post::TYPE_MISSION	=>'Mission' // FIXME! требует миграции...
		],
		$default_table='posts',
		$id_group='Post';
	
	// пока обрабатывает только один случай: до верификации мы знаем только, что событие "Post", а после оно должно стать соответствующим типом работы. новые работы должны создаваться уже с указанным типом. FIXME! потом должно обрабатывать все случая перетекания одного типа работ в другой.
